...
def main():
    ...
    parser.add_argument('--from', type=str, dest='_from',
                        help='decode from', required=True)
    parser.add_argument('--to', type=str, dest='_to',
                        help='encode to', required=True)

    args = parser.parse_args()

    print('inputfile:{}'.format(args.inputfile))
    if (args._from == 'cp950' or args._from == 'big5') and args._to == 'utf8':
        cp950_to_utf8(args.inputfile)
    elif args._from == 'utf8' and (args._to == 'cp950' or args._to == 'big5'):
